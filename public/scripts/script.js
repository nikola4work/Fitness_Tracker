    ...
    $.ajax({
        ...
        success: result => {
            for (exercise of result[0].exercises) {
                $("#exerciseList").append(
                    `<li class="col-6">${exercise.name} (Reps: ${exercise.reps})</li>`
                );
            }
        }
    });
                ...
                success: result => {
                    ...
                    if (result.errors != undefined) {
                        if (result.errors.name) {
                            $(".alert-warning")
                                .text("Please enter the exercise you want to add.")
                                .attr("style", "display:block;");
                        } else if (result.errors.reps) {
                            $(".alert-warning")
                                .text("Please enter the number of reps.")
                                .attr("style", "display:block;");
                        }
        ...
        $.ajax({
            ...
            success: result => {
                ...
                if (result != "Workout validation failed") {
                    ...
                    $(".alert-warning")
                        .text("Please enter the name of your new workout routine.")
                        .attr("style", "display:block;");
                    alertTimeout = setTimeout(() => {
                        $(".alert-warning").attr("style", "display:none;");
                    }, 4000);
